...
const ViewDetails = () => {
    ...
    return (
        <>
            {/* <div className='bg-white md:max-w-[80%] mx-auto my-8 px-[100px] py-[24px] rounded-lg'>
                <h1 className='text-[32px] text-[#0F0F0F] font-extrabold text-center mb-6'>Doctor’s Profile Details</h1>
                <p className="leading-7 px-6  text-[#0F0F0F80] md:text-center">
...
<div className="bg-white max-w-[95%] md:max-w-[80%] mx-auto my-6 md:my-10 px-4 sm:px-6 md:px-10 lg:px-[100px] py-6 md:py-8 rounded-lg shadow-md border border-gray-100">

  <h1 className="text-2xl md:text-3xl font-bold text-[#0F0F0F] text-center mb-6">
    Book an Appointment
  </h1>

  <hr className="mb-4 border-dashed text-[#0F0F0F80]" />

  <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3 sm:gap-0">
    <p className="text-[#0F0F0F] text-base font-medium">
      Availability
    </p>
    <button
  ...
  <div className="mb-6">
    <p className="text-sm text-[#0F0F0F80] leading-relaxed">
  ...
  <button
    className={`btn text-white w-full px-6 py-3 font-semibold text-base ${
      isAvailableToday ? "btn-primary" : "btn-disabled bg-gray-300 text-gray-600 cursor-not-allowed"
    }`}
    disabled={!isAvailableToday}
  >
